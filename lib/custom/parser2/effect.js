...
function handleHashTag(effect) {
    ...
    const matches = effect.match(/#\w+(?=\s*=)/g);
    ...
    if (exist) {
        ...
        console.log("No matching process variable found for:", matches);
        getLogger().log({
            text: "No matching process variable found for:" + matches,
            icon: TimesCircleIcon(),
            scope: getCurrentScope()
        });
    }
...
//         const key = match[1];
//         const value = match[2] !== undefined ? match[2] : match[3]; // pick quoted or unquoted
