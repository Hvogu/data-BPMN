...
export default function Log(
  ...
  eventBus.on(SCOPE_DESTROYED_EVENT, event => {
    ...
    const text = `${isSubProcess ? (getElementName(scopeElement) || 'SubProcess') : 'Process'
  ...
  eventBus.on(TRACE_EVENT, event => {
    ...
    } else if (element.type === 'ta:DataTask') {
      this.log({
        text: elementName || 'Data Task',
        icon: 'bpmn-icon-gateway-parallel',
        scope
      });
      const textarea = document.getElementById(`${element.id}dropsql`);
      const sql = textarea?.value || 'No SQL';
      this.log({
        icon: 'bpmn-icon-gateway-parallel',
        text: `DataTask executed: ${sql}`,
        scope
      });
...
Log.prototype.log = function (options) {
  ...
  const logEntry = domify(`
    <p class="bts-entry ${type} ${scope && this._scopeFilter.isShown(scope) ? '' : 'inactive'
